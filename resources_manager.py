...
class Resources_Manager(object):
    ...
    def update_insert_station(self, new_station):
        ...
        inserted_station = self.stations_collection.update(
        ...
        if not self.database_controller.collection_exists(
            ...
            self.database_controller.create_collection(new_station['internationalCode'], DEFAULT_STATION_INDEXES)

    def update_station(self, new_station):
        station_collection = self.database_controller.get_collection(new_station['internationalCode'])
        if station_collection:
            station_collection.update({'internationalCode': new_station['internationalCode']}, new_station)
    ...
    def insert_measurement(self, station_code, parameter_info, parameter_measurements, date, filename):
        ...
        station_new_measurements_object = self.create_station_object(filename, date, parameter_info, parameter_measurements)
        self.database_controller.update_object_in_collection(station_code, station_new_measurements_object, query_object, True)

    def add_measurements_for_station(self, file_name, station, date, measurements):
        self.update_insert_station(station)
        for parameter_info, parameter_measurements in measurements:
            self.update_insert_parameter(parameter_info)
            self.insert_measurement(station['internationalCode'], parameter_info, parameter_measurements, date, file_name)
